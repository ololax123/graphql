...
`;
    try {
        ...
        const isNegativeRatio =
            user.auditRatio >= 0.1 && user.auditRatio <= 0.9;
...
function drawHorizontalBarChart(skills) {
    ...
    skills.forEach((skill) => {
        if (selectedSkills.includes(skill.type)) {
            if (
                ...
                skill.amount > groupedSkills[skill.type].amount
            ) {
                groupedSkills[skill.type] = {
                    type: skill.type,
                    amount: skill.amount,
    ...
    data.forEach((datum, index) => {
        ...
        text.setAttribute(
            'y',
            index * (barHeight + barPadding) + barHeight / 2
        );
    ...
    const axisRuler = document.createElementNS(
        'http://www.w3.org/2000/svg',
        'line'
    );
...
function drawPieChart(audits) {
    ...
    const data = Object.entries(groupedAudits).map(([type, amount]) => ({
        type,
        amount,
    }));
    ...
    const slices = svg
        ...
        .on('mouseover', function (event, d) {
            ...
            const hoverText = d.data
                ? d.data.type === 'given'
                    ? 'The Percentage of xp you have given to others during audits'
                    : 'The Percentage of xp you have gained from others during your audits'
                : '';
            const text = svg
                .append('text')
    ...
    const labels = svg
        ...
        .text(
            (d) =>
                `${d.data.type} (${Math.round(
                    (d.data.amount / d3.sum(data, (d) => d.amount)) * 100
                )}%)`
        )
        .style('fill', 'white');
...
function drawLineGraph(xpData) {
    ...
    circles
        .append('title') // Use the title element for tooltips
        .text(
            (d) =>
